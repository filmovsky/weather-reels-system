...
# --- Funkcja pobierania mapy online ---
def fetch_map(lat, lon, zoom=12):
    timestamp = int(time.time())
    url = f"https://tile.openstreetmap.org/{zoom}/{int(lon*zoom)}/{int(lat*zoom)}.png"
...
# --- Funkcja pobierania danych pogodowych ---
...
def overlay_weather(map_file, weather_data):
    ...
    output_file = output_dir / f"{Path(map_file).stem}_overlay.png"
    img.save(output_file)
    return str(output_file)

# --- Tworzenie wideo z obrazów ---
def create_video(image_files, output_file="storage/renders/solec_weather_video.mp4"):
    ...
    video = cv2.VideoWriter(str(output_file), fourcc, 1, (width, height))
    for frame in frames:
        video.write(frame)
    video.release()
    return str(output_file)
